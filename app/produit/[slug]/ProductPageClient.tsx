...
export default function ProductPageClient({ initialProduct, initialSettings }: {
  initialProduct: any
  ...
  const [product, setProduct] = useState<Product | null>(
    initialProduct ? {
      ...initialProduct,
      images: (initialProduct.images || []).sort((a: { position: number }, b: { position: number }) => a.position - b.position),
      sections: initialProduct.sections?.[0] || initialProduct.sections || {}
    } : null
  )
    ...
    try {
      ...
      try {
        await fetch('/api/telegram/notify', {
          ...
          body: JSON.stringify({
            orderNum, customerName: formName, customerPhone: phoneComplet,
            productName: product.name, ville: villeFinal, quartier: quartierFinal, total: totalFinal,
          }),
        })
      ...
      const waClean = waRaw.replace('https://wa.me/', '').replace('https://api.whatsapp.com/send?phone=', '').replace(/\+/g, '').replace(/\s/g, '').split('?')[0].split('/message/')[0]
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes popIn { 0%{transform:scale(0.5) translateY(40px);opacity:0} 70%{transform:scale(1.08) translateY(-8px);opacity:1} 100%{transform:scale(1) translateY(0);opacity:1} }
      ...
      <div style={{ maxWidth: 480, margin: '0 auto', width: '100%', background: '#F2F2F7', minHeight: '100dvh', paddingBottom: 140 }}>
        ...
        <div style={{ position: 'relative', background: '#f0f0f0', overflow: 'hidden', userSelect: 'none' }}
          ...
          onTouchEnd={e => {
            ...
            if (Math.abs(diff) > 40 && images.length > 1) { if (diff > 0) nextImage(); else prevImage() }
          }}
        >
          <div style={{ height: 380, position: 'relative', overflow: 'hidden' }}>
            ...
            {product.badge && <div style={{ position: 'absolute', top: 12, left: 12, background: accent, color: '#fff', borderRadius: 20, padding: '4px 12px', fontSize: 12, fontWeight: 800 }}>{product.badge}</div>}
            {disc > 0 && <div style={{ position: 'absolute', top: 12, right: 12, background: '#FF3B30', color: '#fff', borderRadius: 20, padding: '4px 12px', fontSize: 12, fontWeight: 800 }}>-{disc}%</div>}
          </div>
          {images.length > 1 && (
            ...
            <div style={{ padding: '0 16px 36px' }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <input type="text" value={formName} onChange={e => setFormName(e.target.value)} placeholder="Votre nom et prénoms"
                    style={{ ...inputStyle, border: `1.5px solid ${submitted && !formName ? '#FF3B30' : '#E5E5EA'}`, background: submitted && !formName ? '#FFF5F5' : '#FAFAFA' }} />
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <input type="tel" value={phoneNum} onChange={e => setPhoneNum(e.target.value)} onBlur={() => setShowIndicatifs(false)} placeholder="0X XX XX XX XX"
                      style={{ ...inputStyle, flex: 1, border: `1.5px solid ${submitted && !phoneNum ? '#FF3B30' : '#E5E5EA'}`, background: submitted && !phoneNum ? '#FFF5F5' : '#FAFAFA' }} />
                  </div>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div style={{ position: 'relative' }}>
                    <select value={ville} onChange={e => { setVille(e.target.value); setQuartier(''); setQuartierAutre(''); setVilleAutre('') }}
                      style={{ ...selectStyle, border: `1.5px solid ${submitted && !ville ? '#FF3B30' : '#E5E5EA'}`, background: submitted && !ville ? '#FFF5F5' : '#FAFAFA' }}>
                  ...
                  {ville === 'Autre' && (
                    <div style={{ marginTop: 8, padding: '10px 12px', background: accent + '10', borderRadius: 12, border: `2px dashed ${accent}` }}>
                      ...
                      <input type="text" value={villeAutre} onChange={e => setVilleAutre(e.target.value)} placeholder="Ex: Dimbokro, Tiassalé..."
                        style={{ ...inputStyle, border: `1.5px solid ${submitted && !villeAutre ? '#FF3B30' : accent + '44'}`, background: '#fff' }} />
                ...
                {ville && ville !== 'Autre' && (
                  <div style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ position: 'relative' }}>
                      <select value={quartier} onChange={e => { setQuartier(e.target.value); setQuartierAutre('') }}
                        style={{ ...selectStyle, border: `1.5px solid ${submitted && !quartier ? '#FF3B30' : '#E5E5EA'}`, background: submitted && !quartier ? '#FFF5F5' : '#FAFAFA' }}>
                    ...
                    {(quartier === 'Autre' || quartier === 'Autre quartier') && (
                      <div style={{ marginTop: 8, padding: '10px 12px', background: accent + '10', borderRadius: 12, border: `2px dashed ${accent}` }}>
                        ...
                        <input type="text" value={quartierAutre} onChange={e => setQuartierAutre(e.target.value)} placeholder="Ex: Résidence Les Fleurs, Cité Bethel..."
                          style={{ ...inputStyle, border: `1.5px solid ${submitted && !quartierAutre ? '#FF3B30' : accent + '44'}`, background: '#fff' }} />
      ...
      {showSuccess && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 24px', background: 'rgba(0,0,0,.55)', backdropFilter: 'blur(6px)' }}
          ...
          {[...Array(24)].map((_, i) => {
            ...
            return (
              <div key={i} style={{ position: 'fixed', top: '-20px', left: `${(i * 4.2) % 100}%`, width: 6 + (i % 4) * 3, height: 6 + (i % 4) * 3, background: color, borderRadius: ['50%','0%','0%','50%'][i % 4], animation: `confettiFall ${1.8 + (i % 5) * 0.3}s ${(i * 0.08)}s ease-in forwards`, pointerEvents: 'none', zIndex: 201, transform: `rotate(${i * 15}deg)` }} />
            )
          })}
          <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: 28, padding: '36px 28px 28px', textAlign: 'center', width: '100%', maxWidth: 360, position: 'relative', animation: 'popIn .5s cubic-bezier(.32,.72,0,1) forwards', boxShadow: '0 32px 80px rgba(0,0,0,.25)' }}>
            <div style={{ width: 80, height: 80, borderRadius: '50%', background: `linear-gradient(135deg,${accent},${accent}99)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 40, margin: '0 auto 20px', boxShadow: `0 12px 32px ${accent}44`, animation: 'float 2.5s ease-in-out infinite' }}>🎉</div>
            <h2 style={{ fontSize: 22, fontWeight: 900, color: '#0D0D0D', marginBottom: 8, lineHeight: 1.2 }}>Commande confirmée !</h2>
            ...
            <button onClick={() => setShowSuccess(false)} style={{ width: '100%', height: 50, borderRadius: borderBtn, background: `linear-gradient(135deg,${accent},${accent}cc)`, border: 'none', color: '#fff', fontSize: 15, fontWeight: 900, cursor: 'pointer', fontFamily: 'inherit', boxShadow: `0 8px 24px ${accent}44` }}>
