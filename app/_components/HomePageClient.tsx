  ...
  return (
    <div onClick={() => onTap(p)} style={{ background: '#fff', borderRadius, overflow: 'hidden', cursor: 'pointer', boxShadow: '0 2px 14px rgba(0,0,0,.08)' }}
      ...
      <div style={{ position: 'relative', height: 170, background: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
        {img ? <img src={img} alt={p.name} style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', inset: 0 }} loading="lazy" /> : <div style={{ fontSize: 56 }}>🛍️</div>}
...
export default function HomePageClient({ initialSettings, initialProducts, initialCategories }: HomePageClientProps) {
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: var(--font-inter), -apple-system, sans-serif; background: ${bgColor}; }
        h1, h2, h3 { font-family: var(--font-poppins), sans-serif; }
                ...
                return (
                  <div key={p.id} onClick={() => goProduct(p)} style={{ flexShrink: 0, width: 108, cursor: 'pointer' }}>
                    <div style={{ height: 96, borderRadius: 12, background: '#f5f5f5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 44, marginBottom: 6, position: 'relative', overflow: 'hidden' }}>
                      {img ? <img src={img} alt={p.name} loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover', position: 'absolute', inset: 0 }} /> : <span>🛍️</span>}
